...
public class PositioningRulesServiceImpl {
	...
	private List<TradeEvent> getExistingTradeEvent(List<TradeEvent> tradeEvents, Integer tradeId){
	...
	private List<Position> getPositions(Collection<TradeEvent> trades){
		...
		for(TradeEvent t: trades){
			...
			if(position != null){
				...
				position.setTrades(position.getTrades() + (",") + t.getTradeId());
				...
				position.setTrades(t.getTradeId().toString());
